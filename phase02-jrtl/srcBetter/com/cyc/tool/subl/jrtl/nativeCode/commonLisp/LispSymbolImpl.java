...
abstract public class LispSymbolImpl extends AbstractLispObject implements SubLSymbol {
	// Bit flags.
	...
	public  SubLObject thePackage; // Either a package object or NIL.
	volatile protected SubLObject value = SubLSymbol.UNBOUND;
	//protected SubLObject value;
	protected SubLObject function;
	...
	public LispSymbolImpl(String s) {
		...
		this.thePackage = Lisp.NIL;
	}

	public LispSymbolImpl(String s, SubLPackage thePackage) {
		this.name = LispObjectFactory.makeString(s);
	...
	public LispSymbolImpl(SubLString string, int hash, SubLPackage thePackage) {
		this.name = string;
		this.hash = hash;
		this.thePackage = thePackage;
	}

	public LispSymbolImpl(SubLString string, SubLPackage thePackage) {
		this.name = string;
		this.thePackage = thePackage;
	}
	...
	public SubLObject getDescription() {
		...
		try {
			...
			if (this.thePackage instanceof SubLPackage) {
				sb.append(", an ");
				SubLSymbol sym = ((SubLPackage) this.thePackage).findExternalSymbol(this.name);
				sb.append(sym == this ? "external" : "internal");
				sb.append(" symbol in the ");
				sb.append(((SubLPackage) this.thePackage).getJavaName());
				sb.append(" package");
			}
	...
	public SubLObject getParts() {
		...
		parts = parts.push(LispObjectFactory.makeCons("package", this.thePackage));
	...
	public String getQualifiedName() {
		...
		if (this.thePackage == Lisp.NIL)
			return "#:".concat(n);
		if (this.isKeyword())
			return ":".concat(n);
		StringBuilder sb = new StringBuilder(((SubLPackage) this.thePackage).getJavaName());
		if (((SubLPackage) this.thePackage).findExternalSymbol(this.name) != null)
			sb.append(':');
		else
			sb.append("::");
		sb.append(n);
	...
	public boolean isKeyword() {
		return this.thePackage == Lisp.PACKAGE_KEYWORD;
	}
	...
	public String writeToString() {
		...
		if (printReadably)
			if (readtableCase != Keyword.UPCASE || printCase != Keyword.UPCASE) {
				...
				if (this.isKeyword())
					sb.append(':');
				else if (this.thePackage instanceof SubLPackage) {
					sb.append(LispSymbolImpl.multipleEscape(((SubLPackage) this.thePackage).getJavaName()));
					sb.append("::");
				} else
					sb.append("#:");
		...
		if (this.thePackage == currentPackage)
			return symbolName;
		if (currentPackage != null && currentPackage.uses(this.thePackage))
		...
		String packageName = ((SubLPackage) this.thePackage).getJavaName();
