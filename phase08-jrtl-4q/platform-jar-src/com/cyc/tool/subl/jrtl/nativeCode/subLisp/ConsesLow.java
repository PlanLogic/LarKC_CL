...
public class ConsesLow extends SubLTrampolineFile {
	...
	private static SubLObject appendConses(SubLObject[] lists) {
		...
		SubLObject lastLast = null;
		int lastIndex = lists.length - 1;
		for (SubLObject cur : lists)
			if (cur != CommonSymbols.NIL) {
				SubLObject curList = (SubLObject) cur.asConsList().clone();
				if (lastLast == null)
					result = curList;
				else
	...
	public static SubLObject nconc(SubLObject[] lists) {
		...
		if (listCount == 1)
			return lists[0];
		SubLObject cur = CommonSymbols.NIL;
		SubLObject result = CommonSymbols.NIL;
		SubLObject lastLast = CommonSymbols.NIL;
