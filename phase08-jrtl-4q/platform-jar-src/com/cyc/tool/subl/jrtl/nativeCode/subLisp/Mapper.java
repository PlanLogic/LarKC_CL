...
public class Mapper implements SubLFile {
	...
	static {
		Mapper.me = new Mapper();
		Mapper.TRACE_LISTENER = new Snapshot.SnapShootingListener() {
		...
		Mapper.NOT_IMPLEMENTED_FILTER = new Snapshot.SnapShootingFilter() {
	...
	public static SubLObject list_image_index() {
		Mapper.list_image_index_internal();
	...
	public static SubLObject restore_image(SubLObject filename, SubLObject do_full_gc) {
		...
		Mapper.restore_image_internal(filename);
	...
	private static LoadMap writeStateFile(SubLObject subLFileName) throws IOException {
		...
		SubLObject rawStream = CommonSymbols.NIL;
