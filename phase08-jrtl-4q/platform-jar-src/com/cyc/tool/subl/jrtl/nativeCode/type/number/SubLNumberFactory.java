...
public class SubLNumberFactory {
	...
	static {
		SubLNumberFactory.MAX_LONG_VALUE_BIGNUM = new BigInteger("9223372036854775807");
		SubLNumberFactory.MIN_LONG_VALUE_BIGNUM = new BigInteger("-9223372036854775808");
		SubLNumberFactory.FIXNUM_BIT_SIZE = SubLNumberFactory.getFixNumBits();
		SubLNumberFactory.MAX_FIXNUM = SubLNumberFactory.FIXNUM_BIT_SIZE == 32 ? Integer.MAX_VALUE
				: (int) Math.pow(2.0, SubLNumberFactory.FIXNUM_BIT_SIZE - 1) - 1;
		SubLNumberFactory.MIN_FIXNUM = SubLNumberFactory.FIXNUM_BIT_SIZE == 32 ? Integer.MIN_VALUE
				: (int) -Math.pow(2.0, SubLNumberFactory.FIXNUM_BIT_SIZE - 1);
		SubLNumberFactory.FIXNUM_BIT_SIZE_TO_PRECACHE = SubLNumberFactory.getFixNumBitsToPreallocate();
		SubLNumberFactory.MAX_FIXNUM_TO_PRECACHE = (int) Math.pow(2.0,
				SubLNumberFactory.FIXNUM_BIT_SIZE_TO_PRECACHE - 1) - 1;
		SubLNumberFactory.MIN_FIXNUM_TO_PRECACHE = (int) -Math.pow(2.0,
				SubLNumberFactory.FIXNUM_BIT_SIZE_TO_PRECACHE - 1);
		SubLNumberFactory.defaultMaxFixnumToPrecache = (int) Math.pow(2.0, 15.0) - 1;
		SubLNumberFactory.defaultMinFixnumToPrecache = (int) -Math.pow(2.0, 15.0);
		SubLNumberFactory.FIXNUM_MAGNITUDE_TO_PRECACHE = (int) Math.pow(2.0,
				SubLNumberFactory.FIXNUM_BIT_SIZE_TO_PRECACHE);
		SubLNumberFactory.CACHE_SHARD_SIZE = SubLNumberFactory.MAX_FIXNUM_TO_PRECACHE + 1;
		SubLNumberFactory.NUM_CACHE_SHARDS = (int) ((SubLNumberFactory.MAX_FIXNUM + 1L)
				/ SubLNumberFactory.CACHE_SHARD_SIZE);
		SubLNumberFactory.posFixNumCache = new SubLFixnum[SubLNumberFactory.NUM_CACHE_SHARDS][];
		SubLNumberFactory.negFixNumCache = new SubLFixnum[SubLNumberFactory.NUM_CACHE_SHARDS][];
		if (SubLNumberFactory.FIXNUM_BIT_SIZE_TO_PRECACHE <= 16)
			SubLNumberFactory.precacheIntegerRange(SubLNumberFactory.MIN_FIXNUM_TO_PRECACHE,
					SubLNumberFactory.MAX_FIXNUM_TO_PRECACHE);
		else {
			long startMS = System.currentTimeMillis();
			SubLNumberFactory.precacheIntegerRange(SubLNumberFactory.defaultMinFixnumToPrecache,
			...
			System.out.printf("Pre-allocating FIXNUM range [%d,%d] ... this should take %s.\n",
					...
					SubLNumberFactory.goodTimeEstimate(estimatedDuration));
			SubLNumberFactory.precacheIntegerRange(SubLNumberFactory.defaultMaxFixnumToPrecache,
					SubLNumberFactory.MAX_FIXNUM_TO_PRECACHE);
			long trueEndMS = System.currentTimeMillis();
			System.out.printf("FIXNUM pre-allocation took %s.\n",
					SubLNumberFactory.goodTimeEstimate(trueEndMS - startMS));
		}
  //static
	}
