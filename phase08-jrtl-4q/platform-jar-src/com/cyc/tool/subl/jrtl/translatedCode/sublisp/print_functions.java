...
public class print_functions extends SubLTranslatedFile {
	...
	static {
		print_functions.me = new print_functions();
		print_functions.$max_fixnum_base_power$ = null;
		print_functions.$max_fixnum_power_less_one$ = null;
		print_functions.$int0$37 = SubLObjectFactory.makeInteger(37);
		print_functions.$int1$36 = SubLObjectFactory.makeInteger(36);
		print_functions.$sym2$PRINT_OBJECT_BIGNUM_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-BIGNUM-METHOD");
		print_functions.$sym3$PRINT_OBJECT_FIXNUM_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-FIXNUM-METHOD");
		print_functions.$str4$Inf = SubLObjectFactory.makeString("Inf");
		print_functions.$str5$NaN = SubLObjectFactory.makeString("NaN");
		print_functions.$int6$_2 = SubLObjectFactory.makeInteger(-2);
		print_functions.$sym7$PRINT_OBJECT_FLOAT_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-FLOAT-METHOD");
		print_functions.$sym8$PRINT_OBJECT_CHARACTER_METHOD = SubLObjectFactory
				.makeSymbol("PRINT-OBJECT-CHARACTER-METHOD");
		print_functions.$str9$__ = SubLObjectFactory.makeString("#:");
		print_functions.$kw10$INTERNAL = SubLObjectFactory.makeKeyword("INTERNAL");
		print_functions.$kw11$CAPITALIZE = SubLObjectFactory.makeKeyword("CAPITALIZE");
		print_functions.$kw12$UPCASE = SubLObjectFactory.makeKeyword("UPCASE");
		print_functions.$kw13$DOWNCASE = SubLObjectFactory.makeKeyword("DOWNCASE");
		print_functions.$sym14$PRINT_OBJECT_SYMBOL_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-SYMBOL-METHOD");
		print_functions.$sym15$PRINT_OBJECT_STRING_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-STRING-METHOD");
		print_functions.$sym16$QUOTE = SubLObjectFactory.makeSymbol("QUOTE");
		print_functions.$kw17$STREAM = SubLObjectFactory.makeKeyword("STREAM");
		print_functions.$str18$___ = SubLObjectFactory.makeString(" . ");
		print_functions.$str19$____ = SubLObjectFactory.makeString(" ...");
		print_functions.$sym20$PRINT_OBJECT_CONS_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-CONS-METHOD");
		print_functions.$str21$___ = SubLObjectFactory.makeString("...");
		print_functions.$str22$_D = SubLObjectFactory.makeString("~D");
		print_functions.$sym23$PRINT_OBJECT_VECTOR_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-VECTOR-METHOD");
		print_functions.$str24$TEST__A__D__D = SubLObjectFactory.makeString("TEST ~A ~D/~D");
		print_functions.$sym25$PRINT_OBJECT_HASH_TABLE_METHOD = SubLObjectFactory
				.makeSymbol("PRINT-OBJECT-HASH-TABLE-METHOD");
		print_functions.$sym26$PRINT_OBJECT_FUNCTION_METHOD = SubLObjectFactory
				.makeSymbol("PRINT-OBJECT-FUNCTION-METHOD");
		print_functions.$str27$_G = SubLObjectFactory.makeString("#G");
		print_functions.$sym28$PRINT_OBJECT_GUID_METHOD = SubLObjectFactory.makeSymbol("PRINT-OBJECT-GUID-METHOD");
	}
	...
	public static SubLObject escape_vertical_bars(SubLObject string) {
		if (CommonSymbols.NIL == Sequences.find(Characters.CHAR_vertical, string, CommonSymbols.UNPROVIDED,
		...
		try {
			...
			SubLObject i;
			SubLObject curr;
			for (cdotimes_end_var = Sequences.length(string), i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
					.numL(cdotimes_end_var); i = Numbers.add(i, CommonSymbols.ONE_INTEGER)) {
				curr = Strings.sublisp_char(string, i);
				if (CommonSymbols.NIL != Characters.charE(curr, Characters.CHAR_vertical))
		...
		if (CommonSymbols.NIL == print_high.$print_readably$.getDynamicValue(thread)
			...
			try {
				...
				if (CommonSymbols.NIL != print_high.$print_pretty$.getDynamicValue(thread)
					...
					while (!tail.isAtom() && CommonSymbols.NIL == done)
	...
	public static SubLObject print_flonum(SubLObject flonum, SubLObject stream) {
		...
		if (CommonSymbols.NIL != Numbers.infinity_p(flonum))
			...
			if (scale.numGE(print_functions.$int6$_2) && scale.numL(CommonSymbols.EIGHT_INTEGER)) {
				if (scale.isPositive()) {
					...
					for (i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
						...
						streams_high.write_char(
								CommonSymbols.NIL != digit ? Characters.digit_char(digit) : Characters.CHAR_0, stream);
						digits = digits.rest();
					}
					...
					SubLObject cdotimes_end_var;
					SubLObject j;
					for (cdotimes_end_var = Numbers.abs(scale), j = CommonSymbols.NIL, j = CommonSymbols.ZERO_INTEGER; j
				...
				streams_high.write_char(
						CommonSymbols.NIL != digits ? Characters.digit_char(digits.first()) : Characters.CHAR_0,
						stream);
				streams_high.write_char(Characters.CHAR_period, stream);
	...
	public static SubLObject print_symbol(SubLObject symbol, SubLObject stream) {
		...
		if (CommonSymbols.NIL != print_high.$print_escape$.getDynamicValue(thread)
			...
			if (CommonSymbols.NIL != reader.symbol_escapep(symbol)) {
				...
				streams_high.write_string(print_functions.escape_vertical_bars(name), stream, CommonSymbols.UNPROVIDED,
						CommonSymbols.UNPROVIDED);
				streams_high.write_char(Characters.CHAR_vertical, stream);
			} else
				streams_high.write_string(print_functions.escape_vertical_bars(name), stream, CommonSymbols.UNPROVIDED,
			...
			streams_high.write_string(
					print_functions.escape_vertical_bars(
			...
			SubLObject i;
			SubLObject ch;
			for (cdotimes_end_var = Sequences.length(name), i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
					.numL(cdotimes_end_var); i = Numbers.add(i, CommonSymbols.ONE_INTEGER)) {
			...
			name = print_functions.escape_vertical_bars(
			...
			if (pcase_var.eql(print_functions.$kw12$UPCASE))
				streams_high.write_string(CommonSymbols.NIL != lower
				...
				streams_high.write_string(CommonSymbols.NIL != upper
	...
	public static SubLObject print_symbol_package_portion(SubLObject symbol, SubLObject stream) {
		...
		if (symbol.isKeyword())
			...
			thread.resetMultipleValues();
			if (CommonSymbols.NIL == state || !symb.eql(symbol)) {
	...
	public static SubLObject setup_print_functions_file() {
		...
		for (radix = CommonSymbols.NIL, radix = CommonSymbols.TWO_INTEGER; !radix
			...
			done = CommonSymbols.NIL;
			power_less_one = CommonSymbols.NIL;
			divisor = CommonSymbols.NIL;
			new_divisor = CommonSymbols.NIL;
			power_less_one = CommonSymbols.MINUS_ONE_INTEGER;
			divisor = CommonSymbols.ONE_INTEGER;
			new_divisor = radix;
			while (CommonSymbols.NIL == done) {
