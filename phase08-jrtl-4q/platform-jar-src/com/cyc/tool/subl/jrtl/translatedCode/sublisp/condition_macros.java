...
public class condition_macros extends SubLTranslatedFile {
	...
	static {
		condition_macros.me = new condition_macros();
		condition_macros.$list0 = ConsesLow.list(
		...
		condition_macros.$str1$RESTART_TAG = SubLObjectFactory.makeString("RESTART-TAG");
		condition_macros.$str2$DUMMY = SubLObjectFactory.makeString("DUMMY");
		condition_macros.$sym3$CLET = SubLObjectFactory.makeSymbol("CLET");
		condition_macros.$sym4$QUOTE = SubLObjectFactory.makeSymbol("QUOTE");
		condition_macros.$sym5$IGNORE = SubLObjectFactory.makeSymbol("IGNORE");
		condition_macros.$sym6$_RESTARTS_ = SubLObjectFactory.makeSymbol("*RESTARTS*");
		condition_macros.$sym7$CONS = SubLObjectFactory.makeSymbol("CONS");
		condition_macros.$sym8$LIST = SubLObjectFactory.makeSymbol("LIST");
		condition_macros.$list9 = ConsesLow.list(SubLObjectFactory.makeSymbol("*RESTARTS*"));
		condition_macros.$sym10$CCATCH = SubLObjectFactory.makeSymbol("CCATCH");
		condition_macros.$sym11$PROGN = SubLObjectFactory.makeSymbol("PROGN");
		condition_macros.$sym12$_ERROR_HANDLER_ = SubLObjectFactory.makeSymbol("*ERROR-HANDLER*");
		condition_macros.$str13$IGNORE_ERRORS_TAG = SubLObjectFactory.makeString("IGNORE-ERRORS-TAG");
		condition_macros.$kw14$IGNORE_ERRORS_TARGET = SubLObjectFactory.makeKeyword("IGNORE-ERRORS-TARGET");
		condition_macros.$sym15$WITH_ERROR_HANDLER = SubLObjectFactory.makeSymbol("WITH-ERROR-HANDLER");
		condition_macros.$list16 = ConsesLow.list(SubLObjectFactory.makeSymbol("FUNCTION"),
	...
	public static SubLObject with_simple_restart(SubLObject macroform, SubLObject environment) {
		...
		SubLObject body;
		current = body = temp;
		return condition_macros.with_simple_restart_internal(name, format_control, format_arguments, body);
	}
	...
	public static SubLObject with_simple_restart_internal(SubLObject name, SubLObject format_control,
		...
		return ConsesLow.list(condition_macros.$sym3$CLET,
				ConsesLow.list(ConsesLow.list(tag,
						CommonSymbols.NIL != name ? ConsesLow.list(condition_macros.$sym4$QUOTE, name)
