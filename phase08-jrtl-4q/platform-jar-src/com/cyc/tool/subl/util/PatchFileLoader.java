...
public class PatchFileLoader extends ClassLoader {
	...
	static {
		PatchFileLoader.PATCH_FILE_LOADER = PatchFileLoader.class.getClassLoader();
	}
	...
	private static void addPossibleCycLJarFilesToPatches(List<JarFile> jarFiles) {
		...
		while (st.hasMoreElements()) {
			...
			Label_0172: {
				if (!tok.endsWith("lib" + System.getProperty("file.separator") + "cyc.jar")
						&& !tok.endsWith("dist" + System.getProperty("file.separator") + "SubL.jar"))
	...
	public PatchFileLoader(List<JarFile> files) {
		this.classNameToClassMap = new HashMap<String, Class>(1024);
		this.files = files;
