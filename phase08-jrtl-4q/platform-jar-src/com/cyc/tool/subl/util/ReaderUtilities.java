...
public class ReaderUtilities {
	public static int areParenthesesBalanced(String subLStatement) {
		...
		do {
			...
			if (curIndex >= 0) {
				if (!ReaderUtilities.isInsideString(curIndex, subLStatement)
						&& ReaderUtilities.findMatchToOpenParen(curIndex, subLStatement) < 0)
					return 1;
				++curIndex;
			}
		} while (curIndex >= 0);
		...
		do {
			...
			if (curIndex >= 0) {
				if (!ReaderUtilities.isInsideString(curIndex, subLStatement)
						&& ReaderUtilities.findMatchToCloseParen(curIndex, subLStatement) < 0)
					return 2;
				++curIndex;
			}
		} while (curIndex >= 0);
	...
	public static boolean shouldProcessSubLStatementNow(String subLStatement) {
		...
		int parenthesisMatchType = ReaderUtilities.areParenthesesBalanced(subLStatement);
