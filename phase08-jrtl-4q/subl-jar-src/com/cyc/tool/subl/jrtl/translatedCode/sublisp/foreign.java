...
public class foreign extends SubLTranslatedFile {
	...
	public static SubLObject analyze_define_foreign_function_arguments(SubLObject symbol, SubLObject arglist,
		...
		if (CommonSymbols.NIL == Errors.$ignore_mustsP$.getDynamicValue(thread)
				&& CommonSymbols.NIL == valid_arg_conversion_pattern(arg_conversion))
	...
	public static SubLObject declare_foreign_shared_library(SubLObject moniker, SubLObject properties) {
		...
		try {
			...
			if (CommonSymbols.NIL != foreign_shared_library_p(moniker))
	...
	public static SubLObject default_return_value(SubLObject return_type) {
		if (CommonSymbols.NIL == return_type_pointer_p(return_type))
			return CommonSymbols.NIL;
	...
	public static SubLObject define_foreign_function(SubLObject macroform, SubLObject environment) {
		...
		thread.resetMultipleValues();
		return define_foreign_function_internal(symbol, arglist, calling_convention, foreign_shared_library,
	...
	public static SubLObject ensure_foreign_shared_library_loaded(SubLObject moniker, SubLObject options) {
		...
		if (CommonSymbols.NIL == foreign_shared_library_loaded_intP(moniker)) {
			SubLObject release = CommonSymbols.NIL;
			try {
				release = Locks.seize_lock(foreign.$active_foreign_shared_library_lock$.getGlobalValue());
				if (CommonSymbols.NIL == foreign_shared_library_loadedP(moniker)) {
					...
					while (CommonSymbols.NIL != cdolist_list_var) {
						ensure_foreign_shared_library_loaded(dependency, CommonSymbols.UNPROVIDED);
	...
	public static SubLObject find_foreign_shared_library_by_pathname(SubLObject pathname) {
		...
		try {
			release = Locks.seize_lock(foreign.$foreign_shared_library_lock$.getGlobalValue());
			tuple = find_foreign_shared_library_by_property(foreign.$kw4$FILENAME, pathname, CommonSymbols.EQUAL);
		} finally {
			if (CommonSymbols.NIL != release)
				Locks.release_lock(foreign.$foreign_shared_library_lock$.getGlobalValue());
		}
	...
	public static SubLObject foreign_shared_library_installedP(SubLObject moniker) {
		return SubLObjectFactory.makeBoolean(CommonSymbols.NIL != foreign_shared_library_loadedP(moniker)
				|| CommonSymbols.NIL != foreign_shared_library_loadableP(moniker));
	}
	...
	public static SubLObject foreign_shared_library_p(SubLObject moniker) {
		return SubLObjectFactory.makeBoolean(CommonSymbols.NIL != get_foreign_shared_library_pathname(moniker));
	}
	...
	public static SubLObject get_foreign_shared_library_dependencies(SubLObject moniker) {
		return get_foreign_shared_library_property(moniker, foreign.$kw6$DEPENDS_ON, CommonSymbols.NIL);
	}
	...
	public static SubLObject get_foreign_shared_library_pathname(SubLObject moniker) {
		return get_foreign_shared_library_property(moniker, foreign.$kw4$FILENAME, CommonSymbols.NIL);
	}
	...
	public static SubLObject return_type_pointer_p(SubLObject return_type) {
		if (CommonSymbols.NIL != return_type_string_p(return_type))
	...
	static {
		...
		$str9$_S_is_not_associated_with_any_for = SubLObjectFactory
				.makeString("~S is not associated with any foreign shared library.");
		$str10$Shared_library__S___S__is_already = SubLObjectFactory
				.makeString("Shared library ~S (~S) is already loaded.");
		$str11$_S_does_not_designate_one_of_the_ = SubLObjectFactory
				.makeString("~S does not designate one of the libraries (~S) currently loaded.");
		$str12$_S_does_not_designate_a_known_sha = SubLObjectFactory
				.makeString("~S does not designate a known shared library.");
		$str13$_ = SubLObjectFactory.makeString("%");
		$list14 = ConsesLow.list(SubLObjectFactory.makeSymbol("SYMBOL"), SubLObjectFactory.makeSymbol("ARGLIST"),
				SubLObjectFactory.makeSymbol("RULES"));
		$sym15$SYMBOLP = SubLObjectFactory.makeSymbol("SYMBOLP");
		$sym16$LISTP = SubLObjectFactory.makeSymbol("LISTP");
		$list17 = ConsesLow.list(SubLObjectFactory.makeSymbol("&KEY"),
		...
		$list18 = ConsesLow.list(SubLObjectFactory.makeKeyword("CALLING-CONVENTION"),
		...
		$str27$Currently__only_calling_conventio = SubLObjectFactory
				.makeString("Currently, only calling-convention :C is supported.");
		$str28$_FOREIGN_NAME_expects_STRING__not = SubLObjectFactory
				.makeString(":FOREIGN-NAME expects STRING, not ~S.");
		$str29$The_argument_list__S_and_the__ARG = SubLObjectFactory
				.makeString("The argument list ~S and the :ARG-CONVERSION pattern ~S appear malaligned.");
		$str30$_S_is_not_a_valid__ARG_CONVERSION = SubLObjectFactory
				.makeString("~S is not a valid :ARG-CONVERSION argument.");
		$str31$_S_is_not_a_valid__RETURN_TYPE_ar = SubLObjectFactory
				.makeString("~S is not a valid :RETURN-TYPE argument.");
		$str32$Do_not_know_how_to_generate_a_for = SubLObjectFactory
				.makeString("Do not know how to generate a foreign name from ~S for ~S.");
		$list33 = ConsesLow.list(SubLObjectFactory.makeSymbol("SYMBOL"), SubLObjectFactory.makeSymbol("&REST"),
		...
		$list43 = ConsesLow.list(SubLObjectFactory.makeSymbol("ALIEN-NULL-PTR?"), SubLObjectFactory.makeSymbol("%VAL"),
				SubLObjectFactory.makeKeyword("C"));
