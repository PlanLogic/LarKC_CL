...
public class reader extends SubLTranslatedFile {
	public static class $read_token_native extends SubLStructNative {
		public $read_token_native() {
			$pointer = CommonSymbols.NIL;
			$buffer = CommonSymbols.NIL;
			$escapes = CommonSymbols.NIL;
		}
	...
	public static class $readtable_native extends SubLStructNative {
		public $readtable_native() {
			$case = CommonSymbols.NIL;
			$character_syntax = CommonSymbols.NIL;
			$single_byte_char_syntax = CommonSymbols.NIL;
			$macro_functions = CommonSymbols.NIL;
			$single_byte_macro_functions = CommonSymbols.NIL;
			$constituent_traits = CommonSymbols.NIL;
			$single_byte_constit_traits = CommonSymbols.NIL;
			$dispatch_tables = CommonSymbols.NIL;
		}
	...
	public static SubLObject _csetf_read_token_buffer(SubLObject object, SubLObject value) {
		assert CommonSymbols.NIL != read_token_p(object) : object;
	...
	public static SubLObject _csetf_readtable_case(SubLObject object, SubLObject value) {
		assert CommonSymbols.NIL != readtable_p(object) : object;
		return object.setField2(value);
	}
	...
	public static SubLObject backquote_rmf(SubLObject stream, SubLObject ch) {
		...
		try {
			...
			SubLObject flag = backquotify(stream, read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T));
	...
	public static SubLObject backquotify(SubLObject stream, SubLObject code) {
		...
		if (code.isAtom()) {
			...
			if (CommonSymbols.NIL != self_evaluating_atom_p(code))
	...
	public static SubLObject bq_append(SubLObject macroform, SubLObject environment) {
		...
		return bq_cons(reader.$sym189$APPEND, ConsesLow.append(lists, CommonSymbols.NIL));
	}
	...
	public static SubLObject bq_list(SubLObject macroform, SubLObject environment) {
		...
		return bq_cons(reader.$sym187$LIST, ConsesLow.append(objects, CommonSymbols.NIL));
	}
	...
	public static SubLObject bq_listX(SubLObject macroform, SubLObject environment) {
		...
		SubLObject objects;
		SubLObject current = objects = datum;
		return bq_cons(reader.$sym188$LIST_, ConsesLow.append(objects, CommonSymbols.NIL));
	}
	...
	public static SubLObject bq_nconc(SubLObject macroform, SubLObject environment) {
		...
		SubLObject lists;
		SubLObject current = lists = datum;
		return bq_cons(reader.$sym190$NCONC, ConsesLow.append(lists, CommonSymbols.NIL));
	}
	...
	public static SubLObject bq_vector_append(SubLObject macroform, SubLObject environment) {
		...
		return ConsesLow.list(reader.$sym192$BQ_VECTOR,
				bq_cons(reader.$sym193$BQ_APPEND, ConsesLow.append(lists, CommonSymbols.NIL)));
	}
	...
	public static SubLObject character_function(SubLObject ch, SubLObject readtable) {
		...
		if (CommonSymbols.NIL != single_byte_p(ch))
	...
	public static SubLObject character_traits(SubLObject ch, SubLObject readtable) {
		if (readtable == CommonSymbols.UNPROVIDED)
			readtable = reader.$readtable$.getDynamicValue();
		return CommonSymbols.NIL != single_byte_p(ch)
			...
			try {
				...
				if (pcase_var.eql(Characters.CHAR_at))
					result = ConsesLow.cons(reader.$bq_atsign$.getDynamicValue(thread),
							read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T));
	...
	public static SubLObject constituent_rmf(SubLObject stream, SubLObject ch) {
		...
		_csetf_read_token_escapes(token, CommonSymbols.NIL);
		add_to_token(ch, token, CommonSymbols.NIL);
		return Values.values(parse_token(stream, read_token(stream, token, CommonSymbols.NIL)), CommonSymbols.T);
	}
	...
	public static SubLObject constituent_syntaxp(SubLObject ch) {
		SubLThread thread = SubLProcess.currentSubLThread();
		return Equality.eq(
				CommonSymbols.NIL != single_byte_p(ch)
	...
	public static SubLObject create_readtable() {
		...
		_csetf_readtable_dispatch_tables(readtable, CommonSymbols.NIL);
		return readtable;
	}
			...
			while (true) {
				...
				if (pcase_var.eql(reader.$kw185$NORMAL)) {
					if (CommonSymbols.NIL != Characters.charE(next_ch, ch))
						break;
					if (CommonSymbols.NIL == single_escape_syntaxp(next_ch))
						continue;
		...
		while (true) {
			...
			if (pcase_var2.eql(reader.$kw185$NORMAL)) {
				if (CommonSymbols.NIL != Characters.charE(next_ch2, ch))
					break;
				if (CommonSymbols.NIL != single_escape_syntaxp(next_ch2))
					state = reader.$kw186$ESCAPE;
				else
					add_to_token(next_ch2, token, CommonSymbols.NIL);
			} else {
	...
	public static SubLObject features_match(SubLObject feature) {
		...
		if (pcase_var.eql(reader.$kw216$NOT)) {
			...
			return SubLObjectFactory.makeBoolean(CommonSymbols.NIL == features_match(conses_high.cadr(feature)));
		}
		if (pcase_var.eql(reader.$kw218$AND)) {
			...
			while (CommonSymbols.NIL != cdolist_list_var) {
				if (CommonSymbols.NIL == features_match(subfeature))
					return CommonSymbols.NIL;
				cdolist_list_var = cdolist_list_var.rest();
				subfeature = cdolist_list_var.first();
			}
	...
	public static SubLObject frob_case(SubLObject token) {
		...
		if (!pcase_var.eql(reader.$kw147$PRESERVE))
			if (pcase_var.eql(reader.$kw48$UPCASE))
				...
				for (cdotimes_end_var = read_token_pointer(
						token), i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
		...
		if (CommonSymbols.NIL != frobber) {
			...
			SubLObject curr;
			for (cdotimes_end_var2 = read_token_pointer(token), j = CommonSymbols.NIL, j = CommonSymbols.ZERO_INTEGER; j
	...
	public static SubLObject get_macro_character(SubLObject v_char, SubLObject readtable) {
		if (readtable == CommonSymbols.UNPROVIDED)
			readtable = reader.$readtable$.getDynamicValue();
		SubLObject syntax = CommonSymbols.NIL != single_byte_p(v_char)
	...
	public static SubLObject invalid_rmf(SubLObject stream, SubLObject ch) {
		...
		if (CommonSymbols.NIL != reader.$read_suppress$.getDynamicValue(thread)) {
			...
			_csetf_read_token_buffer(token,
					Strings.make_string(reader.$initial_token_size$.getGlobalValue(), CommonSymbols.UNPROVIDED));
			_csetf_read_token_escapes(token, CommonSymbols.NIL);
			return Values.values(read_token(stream, token, CommonSymbols.NIL), CommonSymbols.T);
		}
	...
	public static SubLObject left_paren_rmf(SubLObject stream, SubLObject ch) {
		...
		while (true) {
			...
			if (pcase_var.eql(reader.$kw154$INITIAL)) {
				...
				if (object.eql(reader.$right_paren$.getDynamicValue(thread)))
					simple_reader_error(reader.$str178$Nothing_after_consing_dot_, CommonSymbols.NIL);
				else if (object.eql(reader.$consing_dot$.getDynamicValue(thread)))
					simple_reader_error(reader.$str179$Too_many_consing_dots_, CommonSymbols.NIL);
				else {
					if (CommonSymbols.NIL == reader.$read_suppress$.getDynamicValue(thread))
				...
				if (object.eql(reader.$right_paren$.getDynamicValue(thread)))
					return Values.values(list, CommonSymbols.T);
				simple_reader_error(reader.$str181$Too_many_objects_after_consing_do, CommonSymbols.NIL);
			}
		}
	}
	...
	public static SubLObject make_dispatch_macro_character(SubLObject v_char, SubLObject non_terminating_p,
		...
		set_character_syntax(v_char, CommonSymbols.NIL != non_terminating_p ? reader.$kw61$NON_TERMINATING_MACRO
	...
	public static SubLObject parse_integer(SubLObject string, SubLObject start, SubLObject end, SubLObject radix,
		...
		while (!i.numGE(end)) {
			...
			SubLObject pcase_var = state;
			if (pcase_var.eql(reader.$kw154$INITIAL)) {
				if (CommonSymbols.NIL == whitespace_1_char_p(ch))
				...
				else if (CommonSymbols.NIL != whitespace_1_char_p(ch))
	...
	public static SubLObject parse_number(SubLObject stream, SubLObject token) {
		...
		for (i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
			...
			pcase_var = state;
			if (pcase_var.eql(reader.$kw154$INITIAL)) {
				if (CommonSymbols.NIL != decimal_point_traitp(ch))
					state = reader.$kw155$DOT1;
				else if (CommonSymbols.NIL != plus_sign_traitp(ch))
					state = reader.$kw156$SIGN;
				else if (CommonSymbols.NIL != minus_sign_traitp(ch)) {
				...
				} else if (CommonSymbols.NIL != digit_traitp(ch)) {
					start1 = i;
				...
				if (CommonSymbols.NIL != Characters.digit_char_p(ch, CommonSymbols.TEN_INTEGER)) {
					...
					start1 = i;
					state = reader.$kw158$DIGITS;
				} else if (CommonSymbols.NIL != decimal_point_traitp(ch))
					state = reader.$kw155$DOT1;
				else
					invalid_token_syntax_error(stream, token);
			} else if (pcase_var.eql(reader.$kw157$DECIMALS)) {
				if (CommonSymbols.NIL == Characters.digit_char_p(ch, CommonSymbols.TEN_INTEGER))
					if (CommonSymbols.NIL != digit_traitp(ch))
						state = reader.$kw158$DIGITS;
					else if (CommonSymbols.NIL != exponent_marker_traitp(ch)) {
						end1 = i;
						state = reader.$kw159$EXPONENT;
					} else if (CommonSymbols.NIL != decimal_point_traitp(ch)) {
						end1 = i;
						state = reader.$kw160$DOT2;
					} else if (CommonSymbols.NIL != ratio_marker_traitp(ch)) {
						end1 = i;
				...
				if (CommonSymbols.NIL == digit_traitp(ch))
					if (CommonSymbols.NIL != ratio_marker_traitp(ch)) {
						end1 = i;
						state = reader.$kw161$SLASH;
					} else
						invalid_token_syntax_error(stream, token);
				...
				if (CommonSymbols.NIL != exponent_marker_traitp(ch)) {
				...
				if (CommonSymbols.NIL != plus_sign_traitp(ch))
				...
				simple_reader_error(reader.$str167$Can_t_happen_, CommonSymbols.NIL);
		}
		SubLObject pcase_var2 = state;
	...
	public static SubLObject parse_symbol_or_consing_dot(SubLObject stream, SubLObject token) {
		...
		for (i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
			...
			if (CommonSymbols.NIL != escape_idx && escape_idx.numE(i))
				escapes = escapes.rest();
			else if (CommonSymbols.NIL != package_marker_traitp(ch)) {
				if (CommonSymbols.NIL != pkg_idx2)
					...
					pkg_idx1 = i;
			} else if (CommonSymbols.NIL != dot_traitp(ch))
	...
	public static SubLObject parse_token(SubLObject stream, SubLObject token) {
		...
		if (CommonSymbols.NIL == escapes
				&& CommonSymbols.NIL != potential_number_p(read_token_buffer(token), read_token_pointer(token)))
	...
	public static SubLObject potential_number_p(SubLObject string, SubLObject length) {
		...
		for (i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
			...
			if (CommonSymbols.NIL != digit_traitp(ch) || CommonSymbols.NIL != plus_sign_traitp(ch)
					|| CommonSymbols.NIL != minus_sign_traitp(ch) || CommonSymbols.NIL != ratio_marker_traitp(ch)
					|| CommonSymbols.NIL != decimal_point_traitp(ch)
				...
				if (CommonSymbols.NIL == alphadigit_traitp(ch) || CommonSymbols.NIL != letter_seen_p)
					return CommonSymbols.NIL;
				letter_seen_p = CommonSymbols.T;
			}
		}
		...
		for (j = CommonSymbols.NIL, j = CommonSymbols.ZERO_INTEGER; j
			...
			if (CommonSymbols.NIL != digit_traitp(ch2))
			...
			if (CommonSymbols.NIL != decimal_point_traitp(ch2))
		...
		if (CommonSymbols.NIL == digit_traitp(first) && CommonSymbols.NIL == plus_sign_traitp(first)
				&& CommonSymbols.NIL == minus_sign_traitp(first) && CommonSymbols.NIL == decimal_point_traitp(first)
			...
			return CommonSymbols.NIL;
		if (CommonSymbols.NIL != package_marker_traitp(first))
			return CommonSymbols.NIL;
		SubLObject last = Strings.sublisp_char(string, Numbers.subtract(length, CommonSymbols.ONE_INTEGER));
		if (CommonSymbols.NIL != plus_sign_traitp(last) || CommonSymbols.NIL != minus_sign_traitp(last))
			return CommonSymbols.NIL;
		return CommonSymbols.T;
	}
	...
	public static SubLObject read(SubLObject stream, SubLObject eof_error_p, SubLObject eof_value,
		...
		if (CommonSymbols.NIL != ch && (CommonSymbols.NIL == whitespace_syntaxp(ch) || CommonSymbols.NIL != recursivep))
	...
	public static SubLObject read_delimited_list(SubLObject v_char, SubLObject stream, SubLObject recursivep) {
		...
		while (true) {
			...
			if (CommonSymbols.NIL != Characters.charE(ch, v_char))
				break;
			if (CommonSymbols.NIL != whitespace_syntaxp(ch))
				continue;
			streams_high.unread_char(ch, stream);
			SubLObject next = ConsesLow.cons(read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T),
					CommonSymbols.NIL);
	...
	public static SubLObject read_features(SubLObject stream) {
		...
		try {
			...
			features = read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T);
		} finally {
	...
	public static SubLObject read_number_in_radix(SubLObject stream, SubLObject radix) {
		...
		_csetf_read_token_buffer(token,
				Strings.make_string(reader.$initial_token_size$.getGlobalValue(), CommonSymbols.UNPROVIDED));
		_csetf_read_token_escapes(token, CommonSymbols.NIL);
		read_token(stream, token, CommonSymbols.NIL);
		if (CommonSymbols.NIL != reader.$read_suppress$.getDynamicValue(thread))
			return CommonSymbols.NIL;
	...
	public static SubLObject read_token(SubLObject stream, SubLObject token, SubLObject escapep) {
		...
		while (true) {
			...
			if (CommonSymbols.NIL != escapep) {
				...
				else if (CommonSymbols.NIL != constituent_syntaxp(ch)
						|| CommonSymbols.NIL != non_terminating_macro_syntaxp(ch)
						|| CommonSymbols.NIL != terminating_macro_syntaxp(ch)
						|| CommonSymbols.NIL != whitespace_syntaxp(ch))
					add_to_token(ch, token, CommonSymbols.T);
				else if (CommonSymbols.NIL != single_escape_syntaxp(ch))
				...
				else if (CommonSymbols.NIL != multiple_escape_syntaxp(ch))
					escapep = CommonSymbols.NIL;
				else if (CommonSymbols.NIL != invalid_syntaxp(ch)
				...
				if (CommonSymbols.NIL == ch)
					return token;
				if (CommonSymbols.NIL != constituent_syntaxp(ch)
						|| CommonSymbols.NIL != non_terminating_macro_syntaxp(ch))
					add_to_token(ch, token, CommonSymbols.NIL);
				else if (CommonSymbols.NIL != single_escape_syntaxp(ch))
					add_to_token(streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED,
					...
					if (CommonSymbols.NIL != terminating_macro_syntaxp(ch)
							|| CommonSymbols.NIL != whitespace_syntaxp(ch)) {
						streams_high.unread_char(ch, stream);
						return token;
					}
	...
	public static SubLObject set_syntax(SubLObject ch, SubLObject syntax, SubLObject function) {
		SubLThread thread = SubLProcess.currentSubLThread();
		set_macro_character(ch, function, CommonSymbols.NIL, reader.$standard_readtable$.getDynamicValue(thread));
	...
	public static SubLObject set_syntax_from_char(SubLObject to_char, SubLObject from_char, SubLObject from_readtable,
		...
		if (CommonSymbols.NIL == from_readtable)
			from_readtable = reader.$standard_readtable$.getDynamicValue(thread);
		SubLObject pcase_var;
		SubLObject syntax = pcase_var = CommonSymbols.NIL != single_byte_p(from_char)
	...
	public static SubLObject setup_reader_file() {
		...
		set_macro_character(Characters.CHAR_lparen, reader.$sym77$LEFT_PAREN_RMF, CommonSymbols.NIL,
				reader.$standard_readtable$.getDynamicValue());
		set_macro_character(Characters.CHAR_rparen, reader.$sym78$RIGHT_PAREN_RMF, CommonSymbols.NIL,
				reader.$standard_readtable$.getDynamicValue());
		set_macro_character(Characters.CHAR_quote, reader.$sym79$SINGLE_QUOTE_RMF, CommonSymbols.NIL,
				reader.$standard_readtable$.getDynamicValue());
		set_macro_character(Characters.CHAR_semicolon, reader.$sym80$SEMICOLON_RMF, CommonSymbols.NIL,
				reader.$standard_readtable$.getDynamicValue());
		set_macro_character(Characters.CHAR_quotation, reader.$sym81$DOUBLE_QUOTE_RMF, CommonSymbols.NIL,
				reader.$standard_readtable$.getDynamicValue());
		set_macro_character(Characters.CHAR_backquote, reader.$sym82$BACKQUOTE_RMF, CommonSymbols.NIL,
				reader.$standard_readtable$.getDynamicValue());
		set_macro_character(Characters.CHAR_comma, reader.$sym83$COMMA_RMF, CommonSymbols.NIL,
				reader.$standard_readtable$.getDynamicValue());
	...
	public static SubLObject sharpsign_a_rmf(SubLObject stream, SubLObject ch, SubLObject arg) {
		...
		try {
			reader.$reading_fake_list$.bind(CommonSymbols.T, thread);
			contents = read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T);
		} finally {
			reader.$reading_fake_list$.rebind(_prev_bind_0, thread);
		}
		if (CommonSymbols.NIL != reader.$read_suppress$.getDynamicValue(thread))
	...
	public static SubLObject sharpsign_capital_g_rmf(SubLObject stream, SubLObject ch, SubLObject arg) {
		...
		SubLObject string = read(stream, CommonSymbols.NIL, CommonSymbols.NIL, CommonSymbols.T);
		if (!string.isString())
	...
	public static SubLObject sharpsign_colon_rmf(SubLObject stream, SubLObject ch, SubLObject arg) {
		...
		if (CommonSymbols.NIL != reader.$read_suppress$.getDynamicValue(thread))
			...
			for (i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
				...
				else if (CommonSymbols.NIL != package_marker_traitp(ch_$3))
					invalid_token_syntax_error(stream, token);
				else if (CommonSymbols.NIL != dot_traitp(ch_$3))
					dot_count = Numbers.add(dot_count, CommonSymbols.ONE_INTEGER);
			}
			if (!dot_count.numE(length))
	...
	public static SubLObject sharpsign_dot_rmf(SubLObject stream, SubLObject ch, SubLObject arg) {
		...
		if (CommonSymbols.NIL != arg && CommonSymbols.NIL == reader.$read_suppress$.getDynamicValue(thread))
			unnecessary_argument_error(stream, ch);
		SubLObject subform = read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T);
		if (CommonSymbols.NIL != reader.$read_suppress$.getDynamicValue(thread))
			return Values.values(CommonSymbols.NIL, CommonSymbols.T);
	...
	public static SubLObject sharpsign_minus_rmf(SubLObject stream, SubLObject ch, SubLObject arg) {
		...
		if (CommonSymbols.NIL != arg && CommonSymbols.NIL == reader.$read_suppress$.getDynamicValue(thread))
			unnecessary_argument_error(stream, ch);
		if (CommonSymbols.NIL != read_features(stream)) {
		...
		return Values.values(read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T), CommonSymbols.T);
	}
	...
	public static SubLObject single_quote_rmf(SubLObject stream, SubLObject ch) {
		SubLThread thread = SubLProcess.currentSubLThread();
		SubLObject object = read(stream, CommonSymbols.T, CommonSymbols.NIL, CommonSymbols.T);
	...
	public static SubLObject symbol_escapep(SubLObject symbol) {
		...
		for (i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
			...
			if (!Characters.CHAR_vertical.equalp(ch) && CommonSymbols.NIL == constituent_syntaxp(ch))
				return CommonSymbols.T;
			if (CommonSymbols.NIL != package_marker_traitp(ch))
				return CommonSymbols.T;
			if (CommonSymbols.NIL == dot_traitp(ch))
	...
	public static SubLObject with_read_token(SubLObject macroform, SubLObject environment) {
		...
		return ConsesLow.listS(reader.$sym114$CLET, ConsesLow.list(bq_cons(token, reader.$list139)), reader.$list140,
				reader.$list141, reader.$list142, ConsesLow.append(body, CommonSymbols.NIL));
	}
	...
	public static SubLObject with_special_reader_error_handling(SubLObject macroform, SubLObject environment) {
		...
		SubLObject body;
		SubLObject current = body = datum;
		return bq_cons(reader.$sym116$PROGN, ConsesLow.append(body, CommonSymbols.NIL));
	}
	...
	static {
		...
		$list1 = ConsesLow.list(CommonSymbols.NIL);
		...
		$list199 = ConsesLow.list(SubLObjectFactory.makeSymbol("QUOTE"), CommonSymbols.T, CommonSymbols.NIL);
