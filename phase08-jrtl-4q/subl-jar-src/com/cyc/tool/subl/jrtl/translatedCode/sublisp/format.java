...
public class format extends SubLTranslatedFile {
	...
	public static SubLObject format_a_directive(SubLObject stream, SubLObject string, SubLObject start, SubLObject end,
		...
		try {
			...
			if (CommonSymbols.NIL != parameters) {
				...
				if (CommonSymbols.NIL == format_typep(mincol, format.$list54))
				...
				if (CommonSymbols.NIL == format_typep(colinc, format.$list54))
				...
				if (CommonSymbols.NIL == format_typep(minpad, format.$list54))
				...
				if (CommonSymbols.NIL == format_typep(padchar, format.$sym20$CHARACTER))
	...
	public static SubLObject format_ampersand_directive(SubLObject stream, SubLObject string, SubLObject start,
			...
			SubLObject colonp) {
		check_flags(Characters.CHAR_ampersand, at_sign_p, CommonSymbols.NIL, colonp, CommonSymbols.NIL);
		SubLObject n = CommonSymbols.NIL;
		n = parameters.first();
		SubLObject tail = parameters.rest();
		if (CommonSymbols.NIL == n)
			n = CommonSymbols.NIL;
		if (CommonSymbols.NIL == format_typep(n, format.$list49))
	...
	public static SubLObject format_asterisk_directive(SubLObject stream, SubLObject string, SubLObject start,
		...
		if (CommonSymbols.NIL == format_typep(count, format.$list49))
		...
		if (CommonSymbols.NIL != colonp)
			return Values.values(
					backup_args(args, all_args, CommonSymbols.NIL != count ? count : CommonSymbols.ONE_INTEGER), start);
		if (CommonSymbols.NIL == count)
	...
	public static SubLObject format_cardinal(SubLObject stream, SubLObject integer) {
		...
		if (integer.isZero())
			...
			format_cardinal_internal(stream, integer, CommonSymbols.ZERO_INTEGER, CommonSymbols.NIL);
		return CommonSymbols.NIL;
	}
	...
	public static SubLObject format_circumflex_directive(SubLObject stream, SubLObject string, SubLObject start,
			...
			SubLObject colonp) {
		SubLThread thread = SubLProcess.currentSubLThread();
		check_flags(Characters.CHAR_caret, at_sign_p, CommonSymbols.NIL, colonp, CommonSymbols.T);
		...
		if (pcase_var.eql(CommonSymbols.ZERO_INTEGER)) {
			...
			if (CommonSymbols.NIL == x)
				x = CommonSymbols.NIL;
			if (CommonSymbols.NIL == format_typep(x, format.$sym17$INTEGER))
			...
			if (CommonSymbols.NIL == format_typep(x2, format.$sym17$INTEGER))
			...
			if (CommonSymbols.NIL == y)
				y = CommonSymbols.NIL;
			if (CommonSymbols.NIL == format_typep(y, format.$sym17$INTEGER))
			...
			if (CommonSymbols.NIL == format_typep(x3, format.$sym17$INTEGER))
			...
			if (CommonSymbols.NIL == format_typep(y2, format.$sym17$INTEGER))
			...
			if (CommonSymbols.NIL == z)
				z = CommonSymbols.NIL;
			if (CommonSymbols.NIL == format_typep(z, format.$sym17$INTEGER))
	...
	public static SubLObject format_f_directive(SubLObject stream, SubLObject string, SubLObject start, SubLObject end,
			SubLObject args, SubLObject all_args, SubLObject parameters, SubLObject at_sign_p, SubLObject colonp) {
		SubLThread thread = SubLProcess.currentSubLThread();
		check_flags(Characters.CHAR_F, at_sign_p, CommonSymbols.T, colonp, CommonSymbols.NIL);
		SubLObject _prev_bind_0 = print_high.$print_escape$.currentBinding(thread);
		SubLObject _prev_bind_2 = print_high.$print_readably$.currentBinding(thread);
		try {
			print_high.$print_escape$.bind(CommonSymbols.NIL, thread);
			...
			if (CommonSymbols.NIL == format_typep(k, format.$sym17$INTEGER))
			...
			if (CommonSymbols.NIL == format_typep(overflowchar, format.$list88))
			...
			if (arg.isInteger() || arg.isDouble()) {
				...
				format_integer(stream, arg, CommonSymbols.TEN_INTEGER, ConsesLow.list(w), at_sign_p, CommonSymbols.NIL);
		} finally {
			print_high.$print_readably$.rebind(_prev_bind_2, thread);
			print_high.$print_escape$.rebind(_prev_bind_0, thread);
		}
	...
	public static SubLObject format_i_directive(SubLObject stream, SubLObject string, SubLObject start, SubLObject end,
		...
		if (CommonSymbols.NIL == format_typep(n, format.$list54))
			Errors.error(format.$str32$The__S_parameter__S_is_not_of_typ, format.$sym38$FORMAT, n, format.$list54);
		if (CommonSymbols.NIL != tail)
			Errors.error(format.$str45$Too_many__S_parameters_, format.$sym38$FORMAT);
		check_flags(Characters.CHAR_I, at_sign_p, CommonSymbols.NIL, colonp, CommonSymbols.T);
	...
	public static SubLObject format_integer(SubLObject stream, SubLObject integer, SubLObject radix,
		...
		try {
			...
			if (CommonSymbols.NIL != parameters || CommonSymbols.NIL != colonp) {
				...
				if (CommonSymbols.NIL == format_typep(commachar, format.$sym20$CHARACTER))
				...
				if (CommonSymbols.NIL == format_typep(comma_interval, format.$list55))
	...
	public static SubLObject format_left_brace_directive(SubLObject stream, SubLObject string, SubLObject start,
		...
		if (CommonSymbols.NIL == format_typep(max, format.$list49))
		...
		check_flags(Characters.CHAR_lbrace, expr_end_at_sign_p, CommonSymbols.NIL, expr_end_colonp, CommonSymbols.T);
	...
	public static SubLObject format_left_bracket_directive(SubLObject stream, SubLObject string, SubLObject start,
		...
		check_flags(Characters.CHAR_rbracket, expr_end_at_sign_p, CommonSymbols.NIL, expr_end_colonp,
				CommonSymbols.NIL);
		if (CommonSymbols.NIL != at_sign_p) {
			if (CommonSymbols.NIL != parameters)
				Errors.error(format.$str45$Too_many__S_parameters_, format.$sym38$FORMAT);
			...
			if (CommonSymbols.NIL == separator_start)
				Errors.error(format.$str107$No_clause_marker_found_for__S__S_, Characters.CHAR_lbracket,
						format.$sym38$FORMAT);
			check_flags(Characters.CHAR_semicolon, separator_at_sign_p, CommonSymbols.NIL, separator_colonp,
					CommonSymbols.NIL);
			if (CommonSymbols.NIL != args.first())
			...
			if (CommonSymbols.NIL == format_typep(index, format.$list108))
			...
			while (CommonSymbols.NIL == donep && CommonSymbols.NIL == default_start) {
				...
				thread.resetMultipleValues();
				check_flags(Characters.CHAR_semicolon, separator_at_sign_p2, CommonSymbols.NIL, separator_colonp2,
						CommonSymbols.T);
				if (count.numE(index))
	...
	public static SubLObject format_left_paren_directive(SubLObject stream, SubLObject string, SubLObject start,
		...
		if (CommonSymbols.NIL == expr_end)
			matching_bracket_not_found(Characters.CHAR_lbrace, Characters.CHAR_rbrace, string);
		check_flags(Characters.CHAR_rparen, expr_end_at_sign_p, CommonSymbols.NIL, expr_end_colonp, CommonSymbols.NIL);
	...
	public static SubLObject format_percent_directive(SubLObject stream, SubLObject string, SubLObject start,
			SubLObject end, SubLObject args, SubLObject all_args, SubLObject parameters, SubLObject at_sign_p,
			SubLObject colonp) {
		check_flags(Characters.CHAR_percent, at_sign_p, CommonSymbols.NIL, colonp, CommonSymbols.NIL);
		SubLObject n = CommonSymbols.NIL;
		n = parameters.first();
		SubLObject tail = parameters.rest();
		if (CommonSymbols.NIL == n)
		...
		SubLObject cdotimes_end_var;
		SubLObject i;
		for (cdotimes_end_var = repitition_designator_value(n), i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
				.numL(cdotimes_end_var); i = Numbers.add(i, CommonSymbols.ONE_INTEGER))
			streams_high.terpri(stream);
		return Values.values(args, start);
	}
	...
	public static SubLObject format_question_mark_directive(SubLObject stream, SubLObject string, SubLObject start,
			...
			SubLObject colonp) {
		SubLThread thread = SubLProcess.currentSubLThread();
		check_flags(Characters.CHAR_question, at_sign_p, CommonSymbols.T, colonp, CommonSymbols.NIL);
		if (CommonSymbols.NIL != parameters)
			Errors.error(format.$str45$Too_many__S_parameters_, format.$sym38$FORMAT);
		if (CommonSymbols.NIL != at_sign_p) {
			...
			try {
				...
				if (CommonSymbols.NIL != at_sign_p && CommonSymbols.NIL != colonp) {
					if (arg.isPositive()) {
						...
						format_integer(stream, arg, CommonSymbols.TEN_INTEGER, CommonSymbols.NIL, CommonSymbols.NIL,
								CommonSymbols.NIL);
				} else if (CommonSymbols.NIL != at_sign_p) {
					if (arg.isPositive()) {
	...
	public static SubLObject format_repeat(SubLObject stream, SubLObject ch, SubLObject count) {
		...
		for (cdotimes_end_var = repitition_designator_value(
				count), i = CommonSymbols.NIL, i = CommonSymbols.ZERO_INTEGER; i
						.numL(cdotimes_end_var); i = Numbers.add(i, CommonSymbols.ONE_INTEGER))
	...
	public static SubLObject format_slash_directive(SubLObject stream, SubLObject string, SubLObject start,
			SubLObject end, SubLObject args, SubLObject all_args, SubLObject parameters, SubLObject at_sign_p,
			SubLObject colonp) {
		check_flags(Characters.CHAR_slash, at_sign_p, CommonSymbols.NIL, colonp, CommonSymbols.T);
	...
	public static SubLObject format_t_directive(SubLObject stream, SubLObject string, SubLObject start, SubLObject end,
		...
		if (CommonSymbols.NIL == format_typep(colnum, format.$list54))
	...
	public static SubLObject format_tilde_directive(SubLObject stream, SubLObject string, SubLObject start,
			SubLObject end, SubLObject args, SubLObject all_args, SubLObject parameters, SubLObject at_sign_p,
			SubLObject colonp) {
		check_flags(Characters.CHAR_tilde, at_sign_p, CommonSymbols.NIL, colonp, CommonSymbols.NIL);
		SubLObject n = CommonSymbols.NIL;
		n = parameters.first();
		SubLObject tail = parameters.rest();
		if (CommonSymbols.NIL == n)
	...
	public static SubLObject format_typep(SubLObject object, SubLObject type) {
		if (type.isCons()) {
			...
			if (pcase_var.eql(format.$sym16$OR)) {
				...
				while (CommonSymbols.NIL != cdolist_list_var) {
					if (CommonSymbols.NIL != format_typep(object, distype))
	...
	public static SubLObject format_vertical_bar_directive(SubLObject stream, SubLObject string, SubLObject start,
			SubLObject end, SubLObject args, SubLObject all_args, SubLObject parameters, SubLObject at_sign_p,
			SubLObject colonp) {
		check_flags(Characters.CHAR_vertical, at_sign_p, CommonSymbols.NIL, colonp, CommonSymbols.NIL);
		SubLObject n = CommonSymbols.NIL;
		n = parameters.first();
		SubLObject tail = parameters.rest();
		if (CommonSymbols.NIL == n)
		...
		for (i = start; !i.numGE(end); i = Numbers.add(i, CommonSymbols.ONE_INTEGER)) {
			...
			if (pcase_var.eql(format.$kw128$NORMAL)) {
				...
				if (CommonSymbols.NIL != Characters.charE(ch, Characters.CHAR_hash)) {
					...
					if (CommonSymbols.NIL != format_directive_p(ch))
	...
	static {
		...
		$ic65 = Vectors.vector(new SubLObject[] { CommonSymbols.NIL, SubLObjectFactory.makeString("one"),
		...
		$ic68 = Vectors.vector(new SubLObject[] { CommonSymbols.NIL, CommonSymbols.NIL,
		...
		$ic69 = Vectors.vector(new SubLObject[] { CommonSymbols.NIL, SubLObjectFactory.makeString("one"),
				SubLObjectFactory.makeString("two"), SubLObjectFactory.makeString("three"),
				SubLObjectFactory.makeString("four"), SubLObjectFactory.makeString("five"),
				SubLObjectFactory.makeString("six"), SubLObjectFactory.makeString("seven"),
				SubLObjectFactory.makeString("eight"), SubLObjectFactory.makeString("nine") });
		...
		$ic73 = Vectors.vector(new SubLObject[] { CommonSymbols.NIL, SubLObjectFactory.makeString("tenth"),
		...
		$ic74 = Vectors.vector(new SubLObject[] { CommonSymbols.NIL, SubLObjectFactory.makeString("eleventh"),
		...
		$ic75 = Vectors.vector(new SubLObject[] { CommonSymbols.NIL, SubLObjectFactory.makeString("first"),
