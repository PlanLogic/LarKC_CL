...
public abstract class FixedArityFunctor implements CommonSymbols {
	public interface Arity10Funct {
		SubLObject processItem(SubLObject p0, SubLObject p1, SubLObject p2, SubLObject p3, SubLObject p4, SubLObject p5,
				SubLObject p6, SubLObject p7, SubLObject p8, SubLObject p9);
	}

	public interface Arity6Funct {
		SubLObject processItem(SubLObject p0, SubLObject p1, SubLObject p2, SubLObject p3, SubLObject p4,
				SubLObject p5);
	}
	...
	public interface Arity8Funct {
		SubLObject processItem(SubLObject p0, SubLObject p1, SubLObject p2, SubLObject p3, SubLObject p4, SubLObject p5,
				SubLObject p6, SubLObject p7);
	}

	public interface Arity9Funct {
		SubLObject processItem(SubLObject p0, SubLObject p1, SubLObject p2, SubLObject p3, SubLObject p4, SubLObject p5,
				SubLObject p6, SubLObject p7, SubLObject p8);
	}
	...
	public static SubLObject dump_instances() {
		System.out.println("#|*********************************************************************");
		int totalC = 0;
		for (Map.Entry<String, Integer> e : FixedArityFunctor.signaturesCoded.entrySet()) {
		...
		int totalU = 0;
		for (Map.Entry<String, Integer> e2 : FixedArityFunctor.signaturesUncoded.entrySet()) {
	...
	public static void visitRest(ClassWriter cw, Method method, int arity, int left) {
		...
		MethodVisitor mv = cw.visitMethod(17, "processItem", "(" + args + ")" + FixedArityFunctor.SUBLOBJECT_LNAME,
				null, null);
		mv.visitCode();
		Label l0 = new Label();
		Label l2 = new Label();
		...
		mv.visitMethodInsn(184, FixedArityFunctor.RESOURCER_INAME, "getInstance",
		...
		mv.visitMethodInsn(182, FixedArityFunctor.RESOURCER_INAME, "acquireSubLObjectArray",
		...
		mv.visitVarInsn(25, arity + 2);
		mv.visitMethodInsn(184, Type.getInternalName(method.getDeclaringClass()), method.getName(),
		...
		mv.visitVarInsn(25, arity + 1);
		mv.visitVarInsn(25, arity + 2);
		mv.visitMethodInsn(182, FixedArityFunctor.RESOURCER_INAME, "releaseSubLObjectArray",
	...
	static void visitArityWithOptional(ClassWriter cw, Method method, int arity, int unprovideds, boolean needsRest) {
		...
		while (unprovideds-- > 0)
			mv.visitFieldInsn(178, "com/cyc/tool/subl/jrtl/nativeCode/subLisp/CommonSymbols", "UNPROVIDED",
					"Lcom/cyc/tool/subl/jrtl/nativeCode/type/symbol/SubLSymbol;");
		if (needsRest)
			mv.visitFieldInsn(178, FixedArityFunctor.RESOURCER_INAME, "EMPTY_SUBL_OBJECT_ARRAY",
	...
	static void visitIFacConstructor(ClassWriter cw, Class fixedTypeInterface, String methodNameSerial) {
		...
		String internalName = Type.getInternalName(fixedType);
		...
		cw.visitInnerClass(Type.getInternalName(fixedTypeInterface), FixedArityFunctor.FIX_ARITY_FUNCTOR_INAME,
				fixedTypeInterface.getName(), 1545);
		FieldVisitor fv = cw.visitField(0, "func", FixedArityFunctor.SUBLFUNCTION_LNAME, null, null);
		fv.visitEnd();
		MethodVisitor mv = cw.visitMethod(1, "<init>", "(" + FixedArityFunctor.SUBLFUNCTION_LNAME + ")V", null, null);
		mv.visitCode();
		Label l0 = new Label();
