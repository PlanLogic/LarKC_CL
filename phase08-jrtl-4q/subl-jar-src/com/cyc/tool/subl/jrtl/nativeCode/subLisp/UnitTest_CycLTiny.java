		...
		try {
			SubLProcess subLProcess = new SubLProcess("Initial Lisp Listener") {
				...
				public void safeRun() {
					...
					APPEND_STACK_TRACES_TO_ERROR_MESSAGES.setValue(T);
					...
					if (initForm != null)
						try {
							...
							SubLObject form = reader.read_from_string(initFormString, T, UNPROVIDED, UNPROVIDED,
					...
					if (!SubLMain.shouldRunInBackground()) {
						...
						System.out.println(
								"Total memory allocated to VM: " + (int) (rt.totalMemory() / 1048576.0) + "MB.");
						System.out
								.println("Memory currently available: " + (int) (rt.freeMemory() / 1048576.0) + "MB.");
						System.out.println("Memory currently used: "
								+ (int) ((rt.totalMemory() - rt.freeMemory()) / 1048576.0) + "MB.");
						System.out.println("Running all CycL unit tests.");
						SubLObject form2 = reader.read_from_string(
								SubLObjectFactory.makeString("(run-all-cyc-tests \"/cyc/top/tests/\")"), UNPROVIDED,
