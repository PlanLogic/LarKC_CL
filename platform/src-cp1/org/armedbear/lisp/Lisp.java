...
public class Lisp
{
  ...
  public static final Package PACKAGE_PROLOG =
		    Packages.createPackage("PROLOG");
  public static final Package PACKAGE_SUBLISP =
		    Packages.createPackage("SUBLISP");
  public static final Package PACKAGE_CYC =
		    Packages.createPackage("CYC");
  public static final Package PACKAGE_ASM_JAVA =
		    Packages.createPackage("ASM-JAVA");
  ...
  {
		if (Main.isNoDebug()) {
			throw SubLObjectFactory.makeException(""+condition);
		}
    pushJavaStackFrames();
    if(Main.isSubLisp()) {
    	return (LispObject) Errors.error(condition);
    }
  ...
  public static final LispObject type_error(LispObject abstractSubLObject,
                                            LispObject expectedType)
  {
	  if(Main.isSubLisp()) {
		  Errors.error(""+abstractSubLObject + " not a type "+expectedType);
	  }
    return error(new TypeError(abstractSubLObject, expectedType));
  }
  ...
  public static Symbol checkSymbol(LispObject obj)
  {
	...
	public static LispInteger checkInteger(LispObject obj) {
...
final public static StandardObject checkStandardObject(LispObject first)
  ...
  {
    // ### *gensym-counter*
    Symbol.GENSYM_COUNTER.initializeGlobal(Fixnum.ZERO);
  }
  ...
  // Returns package or throws exception.
  public static final Package coerceToPackage(LispObject obj, Package searchFrom)

  {
    if (obj instanceof Package)
      return (Package) obj;
    if(searchFrom!=null) {
        Package pkg = searchFrom.findPackage(javaString(obj));
        if (pkg != null)
          return pkg;
    }
    Package cp = getCurrentPackage();
    if(cp!=searchFrom) {
        Package pkg = cp.findPackage(javaString(obj));
        if (pkg != null)
          return pkg;
    }
    error(new PackageError(obj.princToString() + " is not the name of a package."));
  ...
  public static LispObject recall(AbstractString key)
  {
                                           ...
                                           LispObject value)
  {
    ...
    symbol.setSpecial(true);
    symbol.setSymbolValue(value);
	//symbol.initializeSpecial(value);
    return symbol;
  }
 ...
 *
 */
public static void checkOutput(Symbol standardOutput, LispObject stdout2) {
	standardOutput.initializeSpecial(stdout2);
	Object o = standardOutput.getSymbolValue();
	if(o!=stdout2) {
		Errors.unimplementedMethod("stdoutput is werld");
	}
    o= standardOutput.symbolValue();
	if(o!=stdout2) {
		Errors.unimplementedMethod("stdoutput is werld");
	}
    o= standardOutput.getValue();
	if(o!=stdout2) {
		Errors.unimplementedMethod("stdoutput is werld");
	}
    //o= standardOutput.getDynamicValue();
	if(o!=stdout2) {
		Errors.unimplementedMethod("stdoutput is werld");
	}
  }
  ...
  {
	  resetIO(Stream.createStream(Symbol.SYSTEM_STREAM, SystemCurrent.in, Symbol.CHARACTER, true),
	    		Stream.createStream(Symbol.SYSTEM_STREAM, SystemCurrent.out, Symbol.CHARACTER, true));
  }
  ...
  {
    resetIO(Stream.createStream(Symbol.SYSTEM_STREAM, ins, Symbol.CHARACTER, true),
    		Stream.createStream(Symbol.SYSTEM_STREAM, outs, Symbol.CHARACTER, true));
  }
 ...
 // ### *lisp-file-type*
 public static final Symbol _LISP_FILE_TYPE_ =
  exportSpecial("*LISP-FILE-TYPE*", PACKAGE_EXT, new SimpleString("lisp"));

 // ### *lisp-file-type-list*
 public static final Symbol _LISP_FILE_TYPE_LIST_ =
  exportSpecial("*LISP-FILE-TYPE-LIST*", PACKAGE_EXT, list(new SimpleString("lisp"),new SimpleString("cl"),new SimpleString("lsp")));
  ...
  static
  {
	...
	public static LispObject loopCheck;

	public static int didExtraInfo;
	 ...
	 *
	 */
	public static void initSystemIO() {
		Stream stdin = Stream.createStream(Symbol.SYSTEM_STREAM, SystemCurrent.in, Symbol.CHARACTER, true);
		Stream stdout = Stream.createStream(Symbol.SYSTEM_STREAM, SystemCurrent.out, Symbol.CHARACTER, true);
		Stream stderr = Stream.createStream(Symbol.SYSTEM_STREAM, SystemCurrent.err, Symbol.CHARACTER, true);
		...
		Symbol.ERROR_OUTPUT.initializeSpecial(stderr);
		Symbol.TRACE_OUTPUT.initializeSpecial(stderr);
  ...
  private static class with_inline_code extends SpecialOperator {
	...
	public static void lisp_type_error(SubLObject subLProcess, String type) {
		if (Main.isSubLisp()) {
			Errors.error("The value: " + valueOfString(subLProcess) + " is not of type " + type + ".");
		}
		Symbol expectedType = Lisp.PACKAGE_CL_USER.findAccessibleSymbol(type);
		type_error(subLProcess.toLispObject(), expectedType);
	}

	public static String getDotName(Class class1) {
		if(class1==null) return "NoClass";
		String firstName = getSimpleName(class1);
		Class sc = class1.getSuperclass();
		String sirName = getSimpleName(sc);
		if(sirName.equals("Object")) return firstName;
		if(firstName.contains(sirName)) return firstName;
		if(sirName.contains(firstName)) return sirName;
		return sirName+"."+firstName;
	}

	public static String getSimpleName(Class class1) {
		if(class1==null) return "NoClass";
		if(class1==Object.class) return "Object";
	    String simpleName  =class1.getSimpleName();
	    if(simpleName!=null && simpleName.length()>0) return simpleName;
	    simpleName = class1.getCanonicalName();
	    if(simpleName!=null && simpleName.length()>0) return simpleName;
	    simpleName = class1.getName();
	    Class sc = class1.getSuperclass();
	    if(sc==Object.class) sc = null;
	    if(sc==null) {
	    	Class[] isc = class1.getInterfaces();
	    	if(isc.length>=0) {
	    		sc=isc[0];
	    	}
	    }
	    String packString = class1.getPackage().getName();
	    if(packString!=null) simpleName = simpleName.substring(packString.length());
	    if(sc!=null) return getSimpleName(sc)+simpleName;
	    return simpleName;
	}
	...
	public static String objectFieldsString(Object thiz) {
		if(thiz==null) 			return "#<?Null>";
		return objectFieldsString(thiz, thiz.getClass().getSuperclass());
	}
	public static String objectFieldsString(Object thiz,Class exceptFor) {
		if(true)return "";
		if (didExtraInfo > 1)
			return "";
		try {
			Lisp.didExtraInfo++;
			return (toString1(JavaObject.getParts(thiz, thiz.getClass(), false, true , Collections.singleton(exceptFor))));
		} finally {
			didExtraInfo--;
		}
	}
	 ...
	 * @param sb
	 */
	static public String toString1(LispObject sb) {
		if (sb == null)
			return "#<?Null>";
		int wasInsideToString = Lisp.insideToString;
		try {
			Lisp.insideToString = 1;
			return sb.printObject();
		} finally {
			Lisp.insideToString = wasInsideToString;
		}
	}
	  static
	  {
