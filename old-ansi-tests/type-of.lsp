...
Expected value: NIL
Actual value: (("" (BASE-STRING)) (#<STANDARD-GENERIC-FUNCTION MEANINGLESS-USER-GENERIC-FUNCTION-FOR-UNIVERSE {26D4C0D5}>  (COMPILED-FUNCTION))).
  ...
  (loop for x in *universe*
	...
	for failures = (loop for tp2 in *cl-all-type-symbols*
			     when (and (typep x tp2)
				       (not (subtypep tp tp2)))
			     collect tp2)
	when failures collect (list x tp failures))
  nil)
...
#|
 (let* ((xx (make-array 0 :element-type nil)) (tp (type-of xx)) (class (class-of xx)))
   (list tp class (multiple-value-list (subtypep tp class))))
