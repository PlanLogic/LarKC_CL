...
public class SpecialOperators {
    ...
    static SubLObject _let(SubLObject args, Environment env,
                                         boolean sequential)
...
for (SubLCons x : nonSequentialVars)
    ...
    static SubLObject _flet(SubLObject args, Environment env,
                                          boolean recursive)
    ...
    private static class sf_progv extends SpecialOperator {
...

        {
            if (args.cl_length() < 2)
                return error(new WrongNumberOfArgumentsException(this));
            LispThread thread = LispThread.currentThread();
            SubLObject symbols = checkList(Lisp.eval(args.first(), env, thread));
